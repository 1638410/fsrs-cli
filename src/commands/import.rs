...
/// 导入命令：从 Markdown 文件或目录解析卡片并导入
pub fn run(source: &Path, base_path: &Path) -> Result<()> {
    ...
    for file in &files {
        let result = import_file(&storage, &search_engine, file, base_path);
        match result {
            Ok((new, updated, skipped)) => {
                total_new += new;
                total_updated += updated;
                total_skipped += skipped;
                total_cards += new + updated;
            }
            Err(e) => {
                eprintln!("{} 导入 {} 失败: {}", style("✗").red(), file.display(), e);
            }
        }
    }

    println!();
    println!("{} 导入完成", style("✓").green().bold());
    println!("  新增: {} 张", total_new);
    println!("  更新: {} 张", total_updated);
    println!("  跳过: {} 张", total_skipped);
    println!("  总计: {} 张", total_cards);
